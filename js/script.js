$(document).ready(function(){
	$.ajax({
		...
		success : function(result){
			$(result.list).each(function(a, b){  //json 데이터의 list 항목에 저장된 값 만큼 함수 내의 실행문을 반복하여 실행. 이 때 매개변수 b에 list 값으로 저장된 객체가 순서대로 할당되도록 진행
				console.log("a:"+a+", b:"+b);  //a는 인덱스번호, b는 인덱스번호에 따른 객체
				$("#wrap ul").append("<li>"+b["k"]+"|"+b["city"]+"</li>");  
